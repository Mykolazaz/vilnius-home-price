# Real Estate Prices in Vilnius

This is a Python data scraping and analysis project with Selenium & Pandas. It is currently in development.

The main goal of this project will be to **predict** real estate prices in Vilnius based on area, floor, location, proximity to amenities and other housing unit characteristics.

The data was scraped from Aruodas.lt, the most popular Lithuanian real estate marketplace.

AI was used strictly only for debugging purposes.